...
with open(sys.argv[1], 'rt') as f:
    for line in f:
        ...
        if line.endswith(':'):
            ...
            continue
        if line.startswith("'"):
            c = bytes(line.strip("'"), 'utf-8').decode('unicodeescape')
    ...
    for instr in program:
        if instr.isdigit() or instr.startswith('-'):
            ...
            if -2**7 <= value < 2**7:
                ...
                f.write(value.tobytes(1, byteorder=byteorder, signed=True))
            elif -2**15 <= value < 2**15:
                f.write(opcode['push2'])
                f.write(value.tobytes(2, byteorder=byteorder, signed=True))
            elif -2**31 <= value < 2**31:
                f.write(opcode['push4'])
                f.write(value.tobytes(4, byteorder=byteorder, signed=True))
            else:
                print >> sys.stderr, "constant too large:", value
                sys.exit(1)
            ...
            f.write(labels[words[1]].tobytes(2, byteorder=byteorder,
                                              signed=False))
        elif instr.startswith('dup'):
            words = instr.split()
            f.write(opcode[words[0]])
            f.write(int(words[1]).tobytes(1, byteorder=byteorder,
                                           signed=False))
        else:
