 ...
 * The size of each array is 4MB, like the stack.
 */

#define HEAP_SIZE 65536
//#define HEAP_SIZE 1250
        ...
        switch (opcode) {
			...
			cons_label:
				...
				if (!reversed) {
					...
					if (flag1) heap1.is_pointer[index] = 1;
					if (flag2) heap1.is_pointer[index + 1] = 1;

				}	
				else {

					index = store(&heap2, data2, data1);
					if (flag1) heap2.is_pointer[index] = 1;
					if (flag2) heap2.is_pointer[index + 1] = 1;
